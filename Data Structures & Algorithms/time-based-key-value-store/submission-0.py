class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
        if key not in self.objects:#this is because we are storing pairs as list which is stored inside a main list [[value,timestamp],.....]
            self.objects[key]=[]
    ...
    def get(self, key: str, timestamp: int) -> str:
        ...
        while l<=r :
            ...
            if values[m][1]<=timestamp : # [m][1] : timestamp 
                ...
                l=m+1 #we are not immediately returning it, we are still updating,if we find any other value later
            else: #if >timestamp
                r=m-1
        return res
